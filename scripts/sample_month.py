#!/usr/bin/env python3
from __future__ import annotations

import argparse

import numpy as np

from fu_alpha_research.config import load_config
from fu_alpha_research.factor_store import FactorStore


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/futures.yaml")
    parser.add_argument("--month", required=True)
    parser.add_argument("--rows", type=int, default=5000)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--force", action="store_true")
    args = parser.parse_args()

    cfg = load_config(args.config)
    out_dir = cfg.output_dir / "samples" / "is"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / f"{args.month}.parquet"
    if out_file.exists() and not args.force:
        print(f"[sample-month] exists {out_file}")
        return

    store = FactorStore(cfg)
    df = store.read_month(args.month, columns=store.selected).dropna(subset=["label"])
    if len(df) > args.rows:
        rng = np.random.default_rng(args.seed + int(args.month.replace("-", "")))
        idx = np.sort(rng.choice(len(df), size=args.rows, replace=False))
        df = df.iloc[idx].copy()
    df.to_parquet(out_file, index=False)
    print(f"[sample-month] wrote {out_file} rows={len(df)}")


if __name__ == "__main__":
    main()
