...
from fu_alpha_research.metrics import add_prediction_views, compute_ic, long_short_backtest, summarize_backtest
...
def summarize_month(cfg, name: str, month: str, force: bool) -> bool:
    part = cfg.output_dir / "prediction_parts" / name / f"{month}.parquet"
    if not part.exists():
        print(f"[summarize-suite] missing {part}")
        return False
    stat_dir = cfg.reports_dir / "prediction_stats"
    stat_dir.mkdir(parents=True, exist_ok=True)
    stat_file = stat_dir / f"{name}_{month}.json"
    bt_file = cfg.output_dir / "backtest_parts" / name / f"{month}.parquet"
    if stat_file.exists() and bt_file.exists() and not force:
        print(f"[summarize-suite] exists {name} {month}")
        return True
    ...
    stat_file.write_text(json.dumps(out, indent=2) + "\n", encoding="utf-8")
    bt = long_short_backtest(df, pred_col="pred_xrank")
    bt_file.parent.mkdir(parents=True, exist_ok=True)
    bt.to_parquet(bt_file, index=False)
    print(f"[summarize-suite] wrote {name} {month}")
    return True


def aggregate_model(cfg, name: str) -> None:
    stat_dir = cfg.reports_dir / "prediction_stats"
    files = sorted(stat_dir.glob(f"{name}_*.json"))
    if not files:
        raise FileNotFoundError(f"no stats files for {name}")
    pred_cols = ["pred", "pred_xsz", "pred_xrank"]
    ...
    pd.DataFrame(summary_rows).to_csv(cfg.reports_dir / f"baseline_summary_{name}.csv", index=False)

    bt_dir = cfg.output_dir / "backtest_parts" / name
    bt_files = sorted(bt_dir.glob("*.parquet"))
    if bt_files:
        bt = pd.concat([pd.read_parquet(path) for path in bt_files], ignore_index=True)
        bt.to_csv(cfg.output_dir / f"backtest_{name}.csv", index=False)
        (cfg.reports_dir / f"backtest_{name}.json").write_text(
            json.dumps(summarize_backtest(bt), indent=2) + "\n",
            encoding="utf-8",
        )
    print(f"[summarize-suite] aggregated {name}")
...
def main() -> None:
    ...
    for prefix in prefixes:
        for set_name in manifest["set"]:
            ...
            seen = [summarize_month(cfg, name, month, args.force) for month in months]
            if any(seen):
                aggregate_model(cfg, name)
